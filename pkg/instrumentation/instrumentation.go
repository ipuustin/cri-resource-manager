...
func InjectGrpcServerTrace(opts ...grpc.ServerOption) []grpc.ServerOption {
	// XXX TODO : for dynamically enable tracing, we should always inject here...
	if !TracingEnabled() {
		return opts
	}
...
func registerJaegerExporter() (*jaeger.Exporter, error) {
	var err error
	...
	if err != nil {
		return nil, traceError("failed to create Jaeger exporter: %v", err)
	}
...
func registerPrometheusExporter() (*prometheus.Exporter, error) {
	...
	if err != nil {
		return nil, traceError("failed to create Prometheus exporter: %v", err)
	}
