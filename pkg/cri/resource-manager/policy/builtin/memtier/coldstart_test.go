...
func TestColdStart(t *testing.T) {
	...
	for _, tc := range tcases {
		t.Run(tc.name, func(t *testing.T) {
			...
			if grant.MemoryType()&memoryDRAM == memoryDRAM {
			...
			time.Sleep(time.Millisecond * time.Duration(tc.expectedColdStartTimeout) * 2)

			newMems := grant.Memset()
			if len(newMems) != 2 {
				t.Errorf("Expected two memory controllers, got: %v", newMems)
			}
